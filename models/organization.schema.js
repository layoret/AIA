...
OrganizationSchema.methods.handleError=function(val){
    console.log("Error");
}
OrganizationSchema.methods.setDate=function(val){
    this.date=val;
}
//Clean extracted title
OrganizationSchema.methods.cleanTitle=function(){
...
OrganizationSchema.methods.cleanBody=function(){
...
OrganizationSchema.methods.setBody=function(val){
    this.bodyHTML=val.html();
    this.bodyText=val.text().replace(/(?:\r\n|\r|\n)/g, '');
    
    
}
OrganizationSchema.methods.findAll=function(cb){
    return this.model('RawArticle').find({},cb);
}
OrganizationSchema.methods.findNoAnalyzed=function(cb){
